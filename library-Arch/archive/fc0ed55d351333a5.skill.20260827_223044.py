...
async def run(api, *, expense_type='Payment on behalf', period='Jun-26', amount='200'):
    await api.click('click')
    await api.click('click-2')
    await api.click('click-3')
    await api.click('click-4')
    await api.select('select', expense_type)
    await api.select('ao-cb-3', period)

...
def _aligned_values(sid: str, sub: Any) -> tuple[dict[str, str], dict[str, Any] | None] | None:
    ...
    if set(by_name.keys()) != tmpl_params:
        ...
        return None
    return by_name, template


# ------------------------------- tier 0: steps -------------------------------


def _instantiated_steps(sid: str, sub: Any) -> list[dict[str, Any]] | None:
    aligned = _aligned_values(sid, sub)
    if aligned is None:
        return None
    ...
    for handle, anchor in anchors.items():
        sels = [
            codegen._TOKEN.sub(
                ...
                s)
            for s in (anchor.get("selectors") or [])
        ]
...
def _load_code_skill(sid: str, sub: Any) -> Skill | None:
...
def load_skill(sid: str, sub: Any) -> Skill | None:
    ...
    trusted to carry the right values (author instead)."""
    skill = _load_code_skill(sid, sub)
    if skill is not None:
        return skill
    steps = _instantiated_steps(sid, sub)
    if steps is None:
        return None
